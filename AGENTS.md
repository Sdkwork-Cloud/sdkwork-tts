...
- Local directories to inspect first when relevant: `.claude/`, `.github/`, `benches/`, `checkpoints/`, `deploy/`, `docs/`, `examples/`, `nginx/`, `scripts/`, `specs/`, `src/`, `tests/`.
...
- `cargo fmt --check`: verify Rust formatting for this package.
- `cargo test`: run Rust tests for this package.
- `cargo clippy --tests -- -D warnings`: lint Rust tests and package code with warnings denied.
